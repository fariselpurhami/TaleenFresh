...
export default function Storefront({ initialProducts }: { initialProducts: Product[] }) {
  ...
  return (
    <div dir="rtl" className="relative min-h-[100dvh] bg-[#f8f9fa] pb-24 overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-80 bg-gradient-to-b from-[#22c55e]/30 via-[#22c55e]/10 to-transparent -z-10" />
      ...
      <div className="px-6 py-2 mb-4">
        <h1 className="text-2xl font-black leading-tight text-foreground mb-1 text-right">
          منتجات طازجة تُقطف بعناية<br />لتصلك بأعلى جودة.
        </h1>
      </div>

      <div className="px-6 mt-2 mb-6 relative z-10">
        <div className="flex items-center gap-3 w-full rounded-2xl bg-white px-4 py-4 shadow-[0_8px_30px_rgb(0,0,0,0.06)] focus-within:ring-2 ring-[#22c55e]/50 transition-all">
          <Search className="text-[#22c55e]" size={20} />
          <input
            type="text"
            value={searchQuery}
            ...
            >
              <motion.div
                ...
                className={`flex h-16 w-16 items-center justify-center rounded-full shadow-sm border-2 transition-colors ${
                  selectedCategory === cat.id ? 'border-[#22c55e] bg-white' : 'border-white ' + cat.color
                }`}
              >
              ...
              <span
                className={`text-xs font-bold transition-colors ${
                  selectedCategory === cat.id ? 'text-[#22c55e]' : 'text-muted-foreground'
                }`}
              >
                {cat.name}
              </span>
      ...
      <div className="px-6 mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs font-bold text-[#22c55e] bg-green-100 px-2 py-1 rounded-md">
      ...
      <div className="fixed bottom-4 left-0 right-0 z-40 mx-auto px-4 w-full max-w-md">
        <div className="flex h-20 items-center justify-between rounded-[2.5rem] bg-white px-8 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100/80 backdrop-blur-md bg-white/90">
          ...
          <div className="relative -top-6">
            <button className="flex h-16 w-16 items-center justify-center rounded-[1.5rem] bg-[#22c55e] text-white shadow-lg shadow-green-500/40 border-4 border-[#f8f9fa] transform rotate-3 hover:rotate-0 transition-transform">
          ...
          <motion.button
            ...
            className="flex flex-col items-center gap-1 text-muted-foreground relative group"
            animate={cartControls} // ربط الحركة بالزر
          >
            <div className="relative">
              <ShoppingCart size={24} strokeWidth={2} className="group-hover:text-[#22c55e] transition-colors" />
              {isMounted && totalItems > 0 && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute -top-1.5 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white shadow-sm ring-2 ring-white"
                >
                  {totalItems}
                </motion.span>
              )}
            </div>
            <span className="text-[10px] font-bold group-hover:text-[#22c55e] transition-colors">السلة</span>
          </motion.button>
        </div>
      </div>
